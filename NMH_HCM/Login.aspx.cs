...
namespace NMH_HCM
{
    public partial class Login : System.Web.UI.Page
    {
        ...
        protected void btnSignIn_ServerClick(object sender, EventArgs e)
        {
            ...
            try
            {
                string query = "select id,Username,UserPassword,Userroles,Active from tblUsers where UserName = @username";
                ...
                if (reader.Read())
                {
                    ...
                    if (reader["UserPassword"].ToString() == txtPassword.Text.Trim())
                    {
                        ...
                        FormsAuthenticationTicket ticket1 = new FormsAuthenticationTicket(
                                   1,                                   // version
                                reader["username"].ToString(),   // get username  from the form
                                DateTime.Now,                        // issue time is now
                                DateTime.Now.AddMinutes(25),         // expires in 25 minutes
